...
function init() {
    ...
    // --- LOW-LEVEL: INTERACTIVITY (ZOOM/PAN) ---
    controls = new THREE.OrbitControls(camera, renderer.domElement);
    controls.minDistance = 500;
    controls.maxDistance = 6000;

    // --- HIGH-LEVEL: GOOGLE LOGIN ---
    window.onload = () => {
...
function getBackgroundColor(worth) {
    ...
    if (val < 100000) return "rgba(239, 48, 34, 0.8)"; // Red (Low)
    if (val > 200000) return "rgba(58, 159, 73, 0.8)";  // Green (High)
    return "rgba(253, 202, 53, 0.8)";                   // Yellow (Mid)
}
...
function createTiles(data) {
    ...
    data.forEach((item, i) => {
        ...
        // --- LOW-LEVEL MATH: DOUBLE HELIX ---
        const hObj = new THREE.Object3D();
        const hPhi = i * 0.175 + (i % 2 === 0 ? 0 : Math.PI); 
...
function animate() {
    ...
    TWEEN.update();
    controls.update(); // Updates zoom and pan
